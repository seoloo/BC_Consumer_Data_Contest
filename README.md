...
공모전에서 제공한 BC Card 원본과 상세 파생 데이터는 저장소에 포함하지 않습니다. `results`에는 발표자료에 이미 제시한 집계 결과만 포함합니다. 서울, 부산, 경기의 외부 데이터도 원본 파일 대신 출처와 처리 방식을 문서화합니다. 노트북 실행에는 사용 권한이 있는 원본 데이터와 NAVER API 인증정보가 필요합니다.

## 공개한 결과 CSV

- 성장군과 비성장군의 Stress 비교
- 지역별 외부유입 민감도 검증 요약
- 규모 기준 민감도 검증
- 통계 검정 결과
- 대표 사례와 4유형 고객 전략
